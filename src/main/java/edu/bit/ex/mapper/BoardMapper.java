...
public interface BoardMapper {
	...
	// 페이징을 적용한 게시글 불러오기
	// @Select("SELECT * FROM (SELECT ROWNUM AS RNUM, A.* FROM (SELECT * FROM mvc_board ORDER BY bGroup DESC, bStep ASC ) A WHERE ROWNUM <= #{pageNum}
	// * #{amount}) WHERE RNUM > (#{pageNum}-1) * #{amount}")
	...
	// 글 작성하기
	// @Insert("INSERT INTO mvc_board (bId, bName, bTitle, bContent, bHit, bGroup, bStep, bIndent) VALUES (mvc_board_seq.nextval, #{bName}, #{bTitle},
	...
	// 답변글 작성하기
	// @Insert("INSERT INTO mvc_board (bId, bName, bTitle, bContent, bGroup, bStep, bIndent) VALUES (mvc_board_seq.nextval, #{bName}, #{bTitle},
